...
import tw.momocraft.inventoryplus.utils.*;
...
public class ConfigHandler {
    ...
    public static FileConfiguration getConfig(String fileName) {
        File filePath = tw.momocraft.inventoryplus.InventoryPlus.getInstance().getDataFolder();
        File file;
        switch (fileName) {
            case "config.yml":
                if (configYAML == null) {
    ...
    private static void getConfigData(File filePath, String fileName) {
        ...
        if (!(file).exists()) {
            try {
                tw.momocraft.inventoryplus.InventoryPlus.getInstance().saveResource(fileName, false);
            } catch (Exception e) {
    ...
    private static void genConfigFile(String fileName) {
        ...
        if (File.exists() && getConfig(fileName).getInt("Config-Version") != configVer) {
            if (tw.momocraft.inventoryplus.InventoryPlus.getInstance().getResource(fileName) != null) {
